...
/** Nút ô caro tự vẽ X/O, tránh lỗi text "..." của GUI Builder/LAF */
public class XOButton extends JButton {
    public final int row;
    public final int col;

    private char mark = '\0'; // '\0' = trống, 'X' hoặc 'O'

    public XOButton(int row, int col) {
        this.row = row;
        this.col = col;

        setText("");                 // không dùng text
    ...
    protected void paintComponent(Graphics g) {
        ...
        try {
            ...
            if (mark == 'X') {
                g2.setColor(new Color(200, 50, 50)); // đỏ cho X
                g2.drawLine(x1, y1, x2, y2);
                g2.drawLine(x1, y2, x2, y1);
            } else if (mark == 'O') {
                g2.setColor(new Color(50, 80, 200)); // xanh cho O
